...
void MotorTask::run() {
    ...
    float zero_electric_offset = 3.21;
    ...
    while (true) {
        ...
        if (xQueueReceive(queue_, &command, 0) == pdTRUE) {
            switch (command.command_type) {
                case CommandType::CONFIG: {
                    ...
                    const float derivative_lower_strength = config.detent_strength_unit * 0.08f;
                    const float derivative_upper_strength = config.detent_strength_unit * 0.02f;
        ...
        if (angle_to_detent_center > config.position_width_radians * config.snap_point &&
            (config.num_positions <= 0 || config.position > 0)) {
            ...
            config.position--;
                   ...
                   (config.num_positions <= 0 || config.position < config.num_positions - 1)) {
            current_detent_center -= config.position_width_radians;
            angle_to_detent_center += config.position_width_radians;
            config.position++;
        }
        ...
        bool out_of_bounds = config.num_positions > 0 && ((angle_to_detent_center > 0 && config.position == 0) ||
                                                          (angle_to_detent_center < 0 &&
                                                           config.position == config.num_positions - 1));
