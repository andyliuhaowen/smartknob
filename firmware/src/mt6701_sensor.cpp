...
float MT6701Sensor::getSensorAngle() {
    ...
    if (now - last_update_ > 100) {
        ...
        uint32_t spi_32 =
                (spi_transaction_.rx_data[0] << 16) | (spi_transaction_.rx_data[1] << 8) | spi_transaction_.rx_data[2];
...
//        uint8_t loss_status = (spi_32 >> 9) & 0x1;
        ...
        if (received_crc == calculated_crc) {
            float new_angle = static_cast<float>(angle_spi) * 2 * static_cast<float>(PI) / 16384;
